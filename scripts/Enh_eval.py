...
def main(test_loader, opt_test, mode):
    if mode == 'eval':
        ...
        for i, data in enumerate(tqdm(test_loader)):
            ...
            torchvision.utils.save_image(restored_SR[0], os.path.join(restored, str(i) + '.png'))
            torchvision.utils.save_image(target_SR[0], os.path.join(target, str(i) + '.png'))
...
if __name__ == '__main__':
    ...
    restored = os.path.join(opt_test['SAVE_DIR'], opt['TRAINING']['MODEL_NAME'], 'restored')
    target = os.path.join(opt_test['SAVE_DIR'], opt['TRAINING']['MODEL_NAME'], 'target')
