...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # -----------------------------------------only 1x1 ------------------------------------------------#
        # temp = self.qkv(x.permute(2, 0, 1)).permute(1, 2, 0)
...
class UnderwaterColorCorrectionNet(nn.Module):
    def __init__(self):
        super(UnderwaterColorCorrectionNet, self).__init__()

        # self.fusion1 = nn.Conv2d(3, 16, 1, 1, bias=True)
        # self.fusion2 = nn.Conv2d(16, 16, 1, 1, bias=True)
        # self.fusion3 = nn.Conv2d(16, 3, 1, 1, bias=True)
        self.fusion = nn.Conv2d(3, 3, 1, 1, bias=True)
    def forward(self, input):

        corrected_image = torch.zeros_like(input)

        for channel in range(3):
            channel_min = torch.min(input[:, channel, :, :])

            channel_max = torch.max(input[:, channel, :, :])
            corrected_channel = (input[:, channel, :, :] - channel_min) / (channel_max - channel_min)
            corrected_image[:, channel, :, :] = corrected_channel

        # Fusion of convolution results
        # output = self.fusion1(corrected_image)
        # output = self.fusion2(output)
        # output = self.fusion3(output)
        output = self.fusion(corrected_image)
...
class URSCT(nn.Module):
    def __init__(self, opt):
        ...
        self.conv1x1 = nn.Conv2d(in_channels=self.in_chans + 1, out_channels=self.in_chans, kernel_size=1, bias=True)
        self.conv_first = nn.Conv2d(in_channels=self.in_chans, out_channels=self.embed_dim, kernel_size=3, stride=1,
                                    padding=1)
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer_up = UpSample(input_resolution=patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                    in_channels=int(self.embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                    scale_factor=2)
            else:
        ...
        self.output = nn.Conv2d(in_channels=self.embed_dim, out_channels=self.out_chans, kernel_size=3, stride=1,
                                padding=1, bias=False)  # kernel = 1
        self.apply(self._init_weights)

        # -------------------------定义一个色彩校正块-----------------------------#
        self.corrcet = UnderwaterColorCorrectionNet()
        self.over = nn.Conv2d(in_channels=18, out_channels=self.out_chans, kernel_size=3, stride=1, padding=1,
                              bias=False)  # kernel = 1
        # --------------------------------------------------------------------#
    ...
    def edge(self, image):
        ...
        # 执行Sobel滤波器操作
        gradient_x = F.conv2d(grayscale_image, torch.from_numpy(sobel_x).unsqueeze(0).unsqueeze(0).float().cuda(),
                              padding=1)
        gradient_y = F.conv2d(grayscale_image, torch.from_numpy(sobel_y).unsqueeze(0).unsqueeze(0).float().cuda(),
                              padding=1)

        # 计算边缘强度图像
        edge_image = torch.sqrt(gradient_x ** 2 + gradient_y ** 2)
        # plt.imshow(edge_image[1].squeeze().cpu().numpy(), cmap='gray')  # 使用灰度颜色映射
        # plt.title("Single Channel Tensor")
        # plt.axis('off')
        # plt.show()
        fusion_image = torch.cat([image, edge_image], 1)
